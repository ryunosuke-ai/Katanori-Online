...
SILENCE_DURATION = 4  # 無音が続いた場合の停止判定時間
SILENCE_DURATIONSEND = 6
...
PC3_SOCKET_URL = "http://192.168.1.65:5010"  # PC3のSocketIOサーバーURLを設定
PC2_RESET_URL = "http://192.168.1.48:5017/reset_silence"  # PC2のリセット用エンドポイント
...
default_posture = np.array([0, 0, -50, 0, 0, 20])
...
def generate_prompt():
    ...
    if talk_count < TALK_MAX: #高齢者とのみの会話
        if talk_count >= TALK_DURATION and talk_count < TALK_MAX:
            response = openai.ChatCompletion.create(
                model="gpt-4o-mini",
                messages=[{"role": "system", "content": "あなたは外の現地にいる人と遠隔で部屋の中にいる二人の会話をサポートするAIです。現在は、遠隔者とのみ会話をしています。出力は、結果のみ返答してください。\n\
                           現地にいる人は慶應義塾大学矢上キャンパスにいます。矢上キャンパスの敷地内を一周することを想定しています。"},
                    {"role": "user", "content": f"今までの高齢者との会話のログは、\n{prompt}\nです。\n\
                     この会話のログを踏まえて、まだ遠隔者と会話をするべきだと判断した場合には0を、そろそろ若者にも話を振って会話を広げていくべきだと判断した場合には1を出力してください。"}],
            ...
            if response["choices"][0]["message"]["content"] == "0": #まだ高齢者と会話をするべきだと判断した場合
                prompt += f"\n遠隔者と会話をしていて、遠隔者の発言は{pc2_transcription}です。\n \
                         ...
                         質問は、上記の会話ログの内容を踏まえた上で、遠隔者に対する質問を1つ生成してください。\n\
                         遠隔者に対する質問を生成する際には、「遠隔者さん、○○（質問内容）」という風に、遠隔者に対して質問していることが分かるように声掛けをしてください。\n\
                ...
                prompt += f"\n現地にいる人と遠隔者が会話をしていて、現在の現地にいる人の発言は{recognized_text}で、遠隔者の発言は{pc2_transcription}です。\n \
                         この発言内容から現在の会話を予測して、入力画像を考慮しながら会話の流れに沿った質問をしてください。\n\
                         入力画像の内容をいまいち読み取れない場合、通常であれば「申し訳ありませんが、～」といった文章も生成していますが、今回は生成した文章をそのまま読み上げようと思っているため、生成する文章は、質問のみにしてください。\n\
                         質問は、上記の会話ログの内容を踏まえた上で、現地にいる人と遠隔者に対する質問を1つずつ生成し、必ず最初に高齢者に対する質問、次に若者に対する質問の順番で生成してください。\n\
                         現地にいる人に対する質問を生成する際には、「遠隔者さん、○○（質問内容）」という風に、遠隔者に対して質問していることが分かるように声掛けをし、\
                         同様に、現地にいる人に対して質問を生成するときも「現地のかた、○○（質問内容）」という風に、現地にいる人に対して質問していることが分かるように声掛けをしてください。\n\
                         返答は、そのまま読み上げてもらうことを想定しているので、「GPTからの応答」など、質問以外の文字は含めず、質問文のみ返答してください。\n"
    ...
    elif talk_count >= TALK_MAX:# 現地の人に話を振る
        prompt += f"\n現地にいる人と遠隔者が会話をしていて、現在の現地にいる人の発言は{recognized_text}で、遠隔者の発言は{pc2_transcription}です。\n \
                         この発言内容から現在の会話を予測して、入力画像を考慮しながら会話の流れに沿った質問をしてください。\n\
                         入力画像の内容をいまいち読み取れない場合、通常であれば「申し訳ありませんが、～」といった文章も生成していますが、今回は生成した文章をそのまま読み上げようと思っているため、生成する文章は、質問のみにしてください。\n\
                         質問は、上記の会話ログの内容を踏まえた上で、現地にいる人に対する質問を1つ生成してください。\n\
                        現地にいる人に対して質問を生成するときは「現地のかた、○○（質問内容）」という風に、若者に対して質問していることが分かるように声掛けをしてください。\n\
                         返答は、そのまま読み上げてもらうことを想定しているので、「GPTからの応答」など、質問以外の文字は含めず、質問文のみ返答してください。\n"
    return prompt
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは外の現地にいる人と遠隔で部屋の中にいる二人の会話をサポートするための質問をするプロの司会者です。\n\
                        現地にいる人には遠隔者と楽しく会話できるような質問を、遠隔者には学校の思い出などを思い出させるような質問をするように心がけてください。\n\
                        "
                },
                {
                    "role": "user",
...
if __name__ == '__main__':
    ...
    # Flaskサーバーを非同期で開始
    threading.Thread(target=lambda: app.run(host='0.0.0.0', port=5005, threaded=True), daemon=True).start() #Threadで並列で実行する場合は、daemon=Trueを全てのスレッドにつければ、Ctrl+Cできちんと終了できる。
